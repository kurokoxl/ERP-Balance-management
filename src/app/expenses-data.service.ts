...
export class ExpensesDataService  {
  ...
  updateExpensesInDatabase(expenses: number): void {
    ...
    // Example:
    this.http.patch(`https://balance-management-c8120-default-rtdb.firebaseio.com/userData/${this.userId}/expenses.json`, { expenses }).subscribe(response => {
      console.log('Expenses updated in the database:', response);
    });
  }
  getExpensesFromDatabase() {
    const url = `https://balance-management-c8120-default-rtdb.firebaseio.com/userData/${this.userId}/expenses.json`;
